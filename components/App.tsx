...
const App: React.FC = () => {
  ...
  const checkAndConsumeCredit = async (): Promise<boolean> => {
      ...
      await authService.updateUserCredits(user.id, newBalance);
      setUser({ ...user, credits: newBalance }); // Update local state
      return true;
  };
  ...
  const handleCreateSubmit = async (data: ResumeFormData) => {
    ...
    try {
      ...
      // Save automatically
      if (user) {
          resumeService.saveResume(user.id, generated);
          triggerSaveToast();
      }
      setResult(generated);
    ...
    <div className="max-w-6xl mx-auto mt-12 px-4">
      ...
      <div className="text-center mb-16 space-y-4">
        <h1 className="text-5xl md:text-6xl font-extrabold text-slate-800 tracking-tight">
          Curriculum <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">Master AI</span>
        </h1>
